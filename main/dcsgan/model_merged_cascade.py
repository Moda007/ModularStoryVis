...
class R2Plus1dStem(nn.Sequential): # Moda: from CP-CSV (NoCascade), used in "VideoEncoder" class
    ...
    def __init__(self):
        super(R2Plus1dStem, self).__init__(
            ...
            nn.BatchNorm3d(64),
            nn.ReLU(inplace=True))
# Not Same Cascade: define_module, sample_videos, sample_images
...
class StoryMartGAN(nn.Module):
    def __init__(self, cfg, video_len):
        ...
        # change configuration # Moda: this part is diff wrt to NoMART and to CP-CSV (NoCascade)
        ##########################
    ...
    def get_iteration_input(self, motion_input): # Moda: no change wrt to NoMART and CP-CSV (NoCascade)
        num_samples = motion_input.shape[0]
        noise = T.FloatTensor(num_samples, self.noise_dim).normal_(0, 1)
        return torch.cat((noise, motion_input), dim=1)

    def get_gru_initial_state(self, num_samples): # Moda: no change wrt to NoMART and CP-CSV (NoCascade)
        return Variable(T.FloatTensor(num_samples, self.motion_dim).normal_(0, 1))

    def sample_z_motion(self, motion_input, video_len=None): # Moda: no change wrt to NoMART (updated) and CP-CSV (NoCascade)
        video_len = video_len if video_len is not None else self.video_len
        num_samples = motion_input.shape[0]
        h_t = [self.m_net(self.get_gru_initial_state(num_samples))]
    ...
    def mart_forward_step(self, prev_ms, input_ids, input_masks): # Moda: Extra Fun for MART
    ...
    def motion_content_rnn(self, word_input_ids_list, input_masks_list, c_code, labels, return_memory=False): # Moda: totally diff from NoMART, it can be used for Transforming info between Seg block also (no priority)
        """
        Args:
