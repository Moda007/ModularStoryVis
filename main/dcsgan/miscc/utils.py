...
class LabelSmoothingLoss(nn.Module): # Moda: DuCo
...
#############################
def KL_loss(mu, logvar): # Moda: match
    # -0.5 * sum(1 + log(sigma^2) - mu^2 - sigma^2)
    KLD_element = mu.pow(2).add_(logvar.exp()).mul_(-1).add_(1).add_(logvar)
    KLD = torch.mean(KLD_element).mul_(-0.5)
    return KLD

def extract_img_features(feature_extractor, input_images, total_seq_len, bsz, video_len): # Moda: DuCo
...
def frame_caption_to_feature(text_input_ids, text_mask, vocab, max_v_len, max_t_len, device): # Moda: DuCo
...
def compute_discriminator_loss(netD, real_imgs, fake_imgs, real_labels, fake_labels, real_catelabels, conditions, gpus, mode='image'): # Moda: Merged
    ...
    if conditions is None: # Moda: this branch comes from CP-CSV. with additional "loss_report" to match DuCo
        ...
        # Moda: Additional branch from CP-CSV, with additional item to "loss_report", to match DuCo
        if netD.seq_consisten_model:
            ...
            loss_report[mode + ' Sequential Consisten Model Loss --> '] = consistency_loss_val # Moda: additional item for "loss_report" to match with DuCo
...
def weights_init(module):
    ...
    if isinstance(module, (nn.Linear, nn.Embedding)): # Moda: for DuCo
        ...
        if classname == "MyInceptionFeatureExtractor": # Moda: for DuCo
            pass
        elif classname == 'BertLayerNorm': # Moda: for DuCo
            module.bias.data.zero_()
...
#############################
def save_img_results(data_img, fake, texts, epoch, image_dir): # Moda: match
    num = cfg.VIS_COUNT
    fake = fake[0:num]
...
##########################\
def images_to_numpy(tensor): # Moda: match
...
def save_story_results(ground_truth, images, texts, idx, image_dir, epoch, lr = False, upscale = False): # Moda: merged, comments diff, and extra return
    ...
    if texts is not None:
        ...
        for idx in range(images.shape[0]):
            ...
            fid.write('\n\n')
        fid.close()
    return all_images # Moda: extra return

def get_multi_acc(predict, real): # Moda: merged, tiny diff in last step of acc calculation
    ...
    for i in range(predict.shape[0]):
        for j in range(predict.shape[1]):
            if real[i][j] == 1 and predict[i][j]>=0.5:
                correct += 1
    acc = correct / (float(np.sum(real)) + sys.float_info.epsilon) # Moda: extra epsilon for DuCo
    return acc

def save_model(netG, netD_im, netD_st, netD_se, epoch, model_dir): # Moda: merged, extra parameter and extra branch for segmentation
    torch.save(
        netG.state_dict(),
    ...
    if netD_se: # Moda: Segmentation branch
        torch.save(
            netD_se.state_dict(),
            '%s/netD_se_epoch_last.pth' % (model_dir))
    print('Save G/D models')

def mkdir_p(path): # Moda: almost match
    try:
        os.makedirs(path)
...
def count_param(model):# Moda: used for segmentation, copied from CP-CSV
    param_count = 0
...
# DAMSM Loss
# ---------------------------------------------------------------------------------
# Moda: this section is just for DuCo
