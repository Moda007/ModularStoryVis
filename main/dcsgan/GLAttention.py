...
class GLAttentionGeneral(nn.Module):
    def __init__(self, idf, cdf):
        ...
        self.linear = nn.Linear(124, idf)
        self.sm = nn.Softmax(dim=-1)
        self.mask = None

    def forward(self, input, sentence, context, mask):
        """
        ...
        # print(sourceT.shape, targetT.shape)
        ...
        if True:
            ...
            if self.mask is not None:
                ...
                attn.data.masked_fill_(mask.data <= 0, -float('inf'))
            ...
            weightedContext = torch.rand(batch_size, idf, ih, iw).cuda()
        ...
        return weightedContext, weightedSentence, None, None
