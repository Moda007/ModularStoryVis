...
from .recurrent import RecursiveTransformer
...
class Translator(object):
    ...
    def translate_batch_beam(self, input_ids_list, video_features_list, input_masks_list, token_type_ids_list,
        ...
        for cur_input_masks in input_masks_list:
            assert torch.sum(cur_input_masks[:, self.max_v_len + 1:]) == 0, \
                ("Initially, all text tokens should be masked??", cur_input_masks[:, self.max_v_len + 1:])
