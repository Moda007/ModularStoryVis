...
class Vocabulary(object): # Moda: Only for DuCo
...
class VideoFolderDataset(torch.utils.data.Dataset): # Moda: match, almost the same as in CP-CSV "datasets/poror.py"
...
class StoryDataset(torch.utils.data.Dataset): # Moda: match, almost the same as in CP-CSV (datasets/poror.py)
...
class ImageDataset(torch.utils.data.Dataset): # Moda: match, almost the same as in CP-CSV (datasets/poror.py). Segmentation parts are added
    def __init__(self, dataset, textvec, image_transform, segment_transform=None, use_segment=False, segment_name='img_segment', return_caption=False): # Moda: merged, Segmentation parameters and part are added
    ...
    def __getitem__(self, item): # Moda: merged, Segmentation parts are added
        ...
        # Moda: read text des (embedding),text attribute (one-hot), raw text, character label (one-hot)
        se = 0
        ...
        if self.use_segment:
            data_item.update({'images_seg': image_seg})
...
class ImageClfDataset(torch.utils.data.Dataset): # Moda: Only for DuCo
...
class StoryImageDataset(torch.utils.data.Dataset): # Moda: Only for DuCo
